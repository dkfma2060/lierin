...
<html>
<head>
<link rel="stylesheet" href="/middle/css/order.css" />
<meta charset="UTF-8">
...
<script type="text/javascript">
	...
	function submitForm(){
		if(${customer.address==null} && document.getElementById("postcode").value == ""){
			...
			form.action = "/middle/customer/payment";
			form.submit();
		}
	}
</script>
...
<body>
	...
	<main>
		...
		<form action="/middle/customer/payment" method="post" name="form">
			...
			<div class="authinfo_area">
				<div class="order_title">받는 분 정보</div>
				<div class="order_title_sub">
					<div class="sub_dl" role="presentation">
						...
						<div class="sub_dd">
							<div>${sessionScope.cAuthInfo.name}
								${sessionScope.cAuthInfo.phone}</div>
							<c:choose>
								...
								<c:otherwise>
									<div class="address_area" id="address_area">
										${customer.address} ${customer.zip_code}</div>
								</c:otherwise>
							</c:choose>
						</div>
					</div>
				...
				<div class="order_title_sub2">
					<div class="sub_dl" role="presentation">
						...
						<div class="sub_dd">
							<div>${sessionScope.cAuthInfo.phone}</div>
						</div>
						<br>
						<br>
						<div class="sub_dt">
						...
						<div class="sub_dd">
							<div>${sessionScope.cAuthInfo.name}/
								${sessionScope.cAuthInfo.email}</div>
						</div>
					</div>
				</div>
			</div>
									...
									<tr class="pay_item_area" id="a">
										<td class="area_image_item">
											<div class="image_unit_item">
												<span class="image_item"> <img
													src="/WUE/img/Simage/${products[i].url}">
												</span>
											</div>
										</td>
										...
										<td class="price_item_quantity">
												...
												<div class="product_price">
													<c:choose>
														<c:when test="${products[i].sale eq 'y'}">
															<div class="cunit_price">
																...
																<div class="product_price">
																	...
																	<span>원</span> <input type="hidden"
																		value="${products[i].sale_price * products[i].quantity}"
																		name="orders[${i}].payment">
																	<c:set var="total"
																		value="${total + products[i].sale_price * products[i].quantity}" />
														...
														<c:otherwise>
															<div class="cunit_price">
																<div class="product_price">
																	<em>${products[i].price * products[i].quantity}</em> <span>원</span>
																	<input type="hidden"
																		value="${products[i].price * products[i].quantity}"
																		name="orders[${i}].payment">
																	<c:set var="total"
																		value="${total + products[i].price * products[i].quantity}" />
																</div>
															</div>
														</c:otherwise>
													</c:choose>
											...
											<div class="codr_unit_amount">
												<em>${products[i].quantity}</em> <span>개</span>
											</div>
										</td>
									</tr>
									<input type="hidden" value="${products[i].pseq}"
										name="orders[${i}].pseq">
									<input type="hidden" value="${products[i].quantity}"
										name="orders[${i}].quantity">
									<input type="hidden" value="${products[i].url}"
										name="orders[${i}].url">
									<input type="hidden" value="${products[i].name}"
										name="orders[${i}].pname">
								</c:forEach>
							</tbody>
						</table>
					...
					<div class="payment_info_area">
						<div class="point_use_area">
							<div class="exist_point">
								가용 포인트 : <span id="exist_point">${customer.point}</span> <em>
									점 </em>
							</div>
							<div class="use_point">
								사용할 포인트 <input type="number" value=0 name="point" id="point"
									pattern="[0-9]+"
									onkeydown="javascript: return event.keyCode == 69 ? false : true"
									onchange="changeamount(${total})">점
							</div>
						</div>
						<div class="order_price">
							...
							<div>
								<span id="total">${total}</span> <em> 원 </em>
							</div>
						</div>
						...
						<div class="payment_method_area">
							...
							<div>결제 방법(신용카드, 카카오페이, 실시간 계좌이체) 카드 종류</div>
						</div>


					</div>
					<input type='hidden' value=${total } name='amount'> <input
						type="button" value="결제하기" onclick="submitForm()"
						onkeydown="javascript: return event.keyCode == 13 ? false : true">
				</div>
			</div>
		</form>
	</main>
