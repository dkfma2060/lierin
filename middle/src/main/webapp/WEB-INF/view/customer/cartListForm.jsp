...
<script type="text/javascript">
	...
	function clickBtn() {
		...
		if(selectedElementsCnt == 0){
			alert("선택된 상품이 없습니다.");
	   		...
	   		form.action = "/middle/customer/cart/delete"; 
	   		form.submit(); 
		}
	}
	...
	function orderBtn(){
		...
		if(total == 0){
			...
			form.action = "/middle/customer/order";
			form.submit();
		}else{
			form.action = "/middle/customer/order";
			form.submit();
...
<body>
	...
	<main>
		<form action="/middle/customer/order" id="form" method='post'>
			<c:set var="size" value="${fn:length(cartList)}" />
			<c:choose>
				<c:when test="${size==0}">
					<div class="cart_insert_message">
						<h2>상품을 장바구니에 추가해보세요.</h2>
					</div>
				</c:when>
				<c:otherwise>
					<div class="cart_list">
						...
						<div class="cart_list_up">
							<input type="checkbox" name="cart" id="cart"
								onclick="selectAll(this,event);"> <span>상품 종류:
								${size} 건</span>
						</div>
										...
										<tr class="pay_item_area" id="a">
											<td class="area_image_item">
												<div class="image_unit_item">
													<span class="image_chk"> <c:choose>
															<c:when test="${cartList[i].total_quantity == 0}">
																<span></span>
															</c:when>
															<c:otherwise>
																<input type="checkbox"
																	id="chk_order_${cartList[i].cseq}" name="cartid"
																	value="${cartList[i].cseq}" onclick="plusPrice(event)">
															</c:otherwise>
														</c:choose> <label for="chk_order_${cartList[i].cseq}"> <span
															class="blind"> ${cartList[i].name} </span> <span
															class="blind"> 상품 선택 </span>
													</label>
													</span> <span class="image_item"> <img
														src="/middle/img/Simage/${imagebyProduct[i]}">
													</span>

												</div>
											</td>
											<td class="name_item"><a
												href="/middle/customer/pseq=${cartList[i].pseq}">${cartList[i].name}</a></td>
														...
														<c:choose>
															...
															<c:when test="${cartList[i].sale eq 'y'}">
																...
																<input type="hidden" name="productPrice"
																	value="${cartList[i].quantity*cartList[i].sale_price}" />
																<c:set var="productPrice"
																	value="${cartList[i].quantity * cartList[i].sale_price}" />
																<c:set var="total"
																	value="${total + cartList[i].quantity * cartList[i].sale_price}" />
																<span>원</span>
															</c:when>
															<c:otherwise>
																<em>${cartList[i].price * cartList[i].quantity}</em>
																<input type="hidden" name="productPrice"
																	value="${cartList[i].quantity*cartList[i].price}" />
																<c:set var="productPrice"
																	value="${cartList[i].quantity * cartList[i].price}" />
																<c:set var="total"
																	value="${total + cartList[i].quantity * cartList[i].price}" />
																<span>원</span>
					...
					<div style="margin: 20px;">
						<b>총액 </b><input type="text" name="totalPrice" id="totalPrice"
							value="${total}" disabled /> <input type="button" value="주문하기"
							onclick="orderBtn();"> <input type="button" value="삭제"
							onclick="clickBtn();">
					</div>
				</c:otherwise>
			</c:choose>
