...
<script type="text/javascript">
	function cancelBtn(odseq, oseq, payment){
		form.action = "/middle/customer/order/delete/odseq="+odseq+"&oseq="+oseq+"&payment="+payment;
		form.submit();
	}
</script>
</head>
			...
			<main class="main_area">
				<article style="padding-left: 50px;">
					<div>
						...
						<h2>${sessionScope.cAuthInfo.grade} 등급</h2>
										...
										<td class="price_item_quantity">
											<div class="cunit_price">
												<div class="product_price">
													<span class="paymeny"> <em>${order.payment}</em> <span>원</span>
													</span>
												</div>
											</div>
											<div class="codr_unit_amount">
												<em>${order.quantity}</em> <span>개</span>
											</div>
										</td>
										...
										<td class="order_delete_area">
											...
											<input type="hidden" value="${order.payment}" name="payment"> --%>
															<input type="button" value="주문 취소"
																onclick="cancelBtn(${order.odseq}, ${order.oseq}, ${order.payment});">
														</div>
													</c:when>
													<c:otherwise>
														<em>주문 취소 불가능</em>
