...
<script type="text/javascript">
	...
	function logoutBtn() {
		form.action = "/middle/customer/logout";
		form.submit();
	}
</script>
</head>
<body>
	...
	<main class="main_area">
		<article style="padding-left: 50px;">
			<div>
				<h1>${sessionScope.cAuthInfo.name}님</h1>
				<h2>point : ${customer.point}점</h2>
			</div>
			<h3>회원 정보 변경</h3>
			<div class="info_all_area">
				<div class="password_area"></div>
				<div class="address_area"></div>
			</div>

		</article>
		<form name="form" action="change" method="post"
			...
			<fieldset style="width: 600px; position: relative;">
				<table border="0" align=center cellspacing=1 bordercolor=white
					bordercolorlight=white bgcolor=white>
					<tr>
					...
					<tr>
						...
						<td><input type=text id='email' name=email size=12
							maxlength=12 readonly="readonly" value="${customer.email}"
							style="width: 150px;">(이메일은 수정 불가능합니다.)
					</tr>
					<tr>
						<td colspan=1 bgcolor=MistyRose align=center>이름</td>
						<td><input type=text id='name' name='name' size=12
							maxlength=20 value="${customer.name}" pattern="^[가-힣]*{2,10}">&nbsp;(한글
							2자~10자로 입력해주세요)</td>
					</tr>

					<tr>
						<td bgcolor=MistyRose align=center>비밀번호 재설정</td>
						<td><input type=password id='pw' name=pw size=12 maxlength=20
					...
					<tr>
						...
						<td><input type=text id='phone1' name=phone1 size=4
							maxlength=3 value="${tel[0]}" pattern="[0-9]{3}">&nbsp;-&nbsp;
							<input type=text id='phone2' name=phone2 size=4 maxlength=4
							value="${tel[1]}" pattern="[0-9]{3,4}">&nbsp;-&nbsp; <input
							type=text id='phone3' name=phone3 size=4 maxlength=4
							value="${tel[2]}" pattern="[0-9]{4}"></td>
					</tr>
					<tr>
						<td colspan=2 height=2 background="dot.gif"></td>
					</tr>
					<tr>
						...
						<td><input type=text name=zip_code size=3 maxlength=3
							id="post" value="${customer.zip_code}" style="width: 100px;">
							<img style="width: 3%; vertical-align: middle;" id='dc'
							src="/middle/image/search.png"
					...
					<tr>
						...
						<td><input type=text name=address value="${customer.address}"
							size=40 maxlength=50 id="add" style="width: 300px;"> <span
							id="guide" style="color: #999; display: none"></span></td>
					</tr>

					<tr>
						<td colspan=2 bgcolor=white align=center height=30><input
							type=submit value='수정하기'> <input type="button" value='취소'
							onclick='location.href= "main"' /> <input type="button"
							value='탈퇴하기' onclick=' checkDrop()' /></td>
					</tr>
				</table>
			</fieldset>
		</form>
