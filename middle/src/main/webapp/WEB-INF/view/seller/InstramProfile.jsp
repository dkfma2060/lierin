		...
		<div class="profile" style="position: relative; height: 150%;">
			<div class="title_image">
				...
				<div class="txt">
					<c:choose>
						<c:when test="${fram=='[]'}">
							<span style="position: relative; left: 40%">${userPostNumber.size()}</span>
							<br>
							<span>게시물</span>
						</c:when>
						<c:otherwise>
							<span style="position: relative; left: 40%">${userPostNumber.size()+1}</span>
							<br>
							<span>게시물</span>
						</c:otherwise>
					</c:choose>
				...
				<div class="txt">
					<span style="position: relative; left: 40%">0</span><br> <span>팔로워</span>
				</div>

				<div class="txt">
					<span style="position: relative; left: 40%">0</span><br> <span>팔로잉</span>
				</div>
			</div>

			<div class="comment"
				style="border: 2px solid white-space; width: 400px; height: 25%; position: relative; left: 70px; top: -30px; overflow: hidden;">
			...
			<c:choose>
				<c:when test="${user.seller_email==authInfo.email}">
					<div class="fix"
						...
						<a href="instagramProfileUpdate?seller_email=${user.seller_email}"
							class="btn btn-default">프로필 수정</a>
					</div>
					<c:choose>
						<c:when test="${fram=='[]'}">
					...
					<c:choose>
						<c:when test="${framimage!='[]'}">
							<a href="instagramSelectfarm?email=${user.seller_email}"> <img
								src="/middle/img/${framimage[0]}"
								style="width: 200px; height: 200px; display: run-in; margin-top: 25px; margin-left: 10px;" /></a>
							<span
								style="position: absolute; border: 2px solid gray; top: 26px; left: 11px; color: blue; background-color: white">주말농장</span>
						</c:when>
					</c:choose>
					<c:choose>
						<c:when test="${userPostNumber.size()!=0}">
							<c:forEach var="i" begin="0" end="${userPostNumber.size()-1}">
								<a
									href="instagramSelectPost?postnumber=${userPostNumber[i]}&email=${user.seller_email}">
									<img src="/middle/img/${imageFrist[i]}"
									style="width: 200px; height: 200px; display: run-in; margin-top: 25px; margin-left: 10px;" />
								</a>
							</c:forEach>
						</c:when>
					</c:choose>
