				...
				<table class="register" border=1 align=center cellspacing=1
					...
					<c:forEach var="i" begin="0"
						...
						<tr>
							...
							<td width=150 align=center><input type="date"
								...
								value="${posting2[0].getFramList().get(i).fseq}"> <c:set
									value="${posting2[0].getFramList().get(i).fseq}" var="fseq"></c:set>
							</td>


							<td width=150 align=center><input type="time"
								name="experience_time"
								value="${posting2[0].getFramList().get(i).experience_time}">
							</td>


							<td width=150 align=center><input type=text id='name'
								name='experience_price' size=12 maxlength=10
								value="${posting2[0].getFramList().get(i).experience_price}">
							</td>


							<td width=150 align=center><input type=text id='title'
								name='experience_number' size=10 maxlength=30
							...
							<c:when test="${posting2[0].getFramList().get(i).fseq!=posting2[0].getFramList().get(0).fseq}"> --%>
								...
								onclick="location.href='deltefarm?fseq=${posting2[0].getFramList().get(i).fseq}'">
								<%--</c:when>
							 <c:otherwise>
					...
					<tr>
						...
						<td colspan=5><div id="detail">
								...
								<c:forEach var="i" begin="0"
									...
									<c:choose>
										<c:when
											test="${posting2[0].getFramList().get(i).fseq==samllFseq}">
											<textarea rows="" cols="" id="detail"
												name="experience_context" size=200 maxlength=250
					...
					<tr>
						<td colspan=5 align=center height=30><input type=submit
							value=게시물수정> <input type="button"
							onclick="delteFramAll()" value="전체 삭제"> <input
							type="button" value=취소 onclick="javascript:history.back();"></td>
					</tr>
					<tr>
						<td colspan=5 height=2 background="dot.gif"></td>
					</tr>
